...
public:
  ...
  void request(Locomotive &loco, int priority) override {
    ...
    afficher_message(
        qPrintable(QString("Locomotive %1 demande l'accès avec priorité %2.")
                       .arg(loco.numero())
                       .arg(priority)));
    mutexSection.release();
    semaphoreSection
        .release(); // Indique qu'une nouvelle demande est disponible
  }
  ...
  void access(Locomotive &loco, int priority) override {
    ...
    while (true) {
      ...
      if (!isUsed && canAccess(priority, loco.numero())) {
        ...
        afficher_message(
            qPrintable(QString("Locomotive %1 accède à la section partagée.")
                           .arg(loco.numero())));
        mutexSection.release();
        break;
      }
  ...
  void leave(Locomotive &loco) override {
    ...
    afficher_message(
        qPrintable(QString("Locomotive %1 quitte la section partagée.")
                       .arg(loco.numero())));
...
private:
  ...
  PriorityMode priorityMode =
      PriorityMode::HIGH_PRIORITY; // Mode de gestion des priorités
  PcoSemaphore
      mutexSection; // Sémaphore pour protéger l'accès aux ressources critiques
  PcoSemaphore
      semaphoreSection; // Sémaphore pour gérer les locomotives en attente
  std::priority_queue<std::pair<int, int>>
      requests; // File des demandes d'accès (priorité, ID locomotive)
};
